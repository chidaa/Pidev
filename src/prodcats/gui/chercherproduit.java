 ...
 * @author USER
 */
public class chercherproduit extends Form { 
    Resources theme;
   String chercher;
    private ContainerList listContainer;
    

    public chercherproduit(Resources theme,String chercher) {
        this.theme=theme;
     ...
     ArrayList<Produits> allmedecins = ms.getProduits(chercher);
        for (Produits m : allmedecins){
       //AFFICHER Image
       ...
       getToolbar().addCommandToLeftSideMenu("Retour", null, (evt) -> {
           AffProduit f= new  AffProduit(theme);
           f.show();
        });
